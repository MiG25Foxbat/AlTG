 ...
 *
 * ВАЖНО: public_posts — общий кэш, читаемый любым пользователем, у
 * которого канал активен, поэтому сюда можно писать только посты из
 * действительно ПУБЛИЧНЫХ каналов. Будущая фича чтения приватных каналов
 * НЕ должна переиспользовать эту функцию для приватного контента — для
 * этого в схеме уже есть отдельная таблица private_posts, скоупленная по
 * user_id.
 */
